...
class ProcessAbst:
    ...
    # Important note!: Every instance that inherit this class should class these method in their constructor/destructor
    ...
    def Initialize(self, progArg, globalConfig):
        ...
        self._processName = self._progArg.GetProcessName()
        ...
        self._comm.RegisterChannel(self._processName, self._processName)
        self._comm.SetChannelBufferSize(self._processName, self._processName, 10)

        self._properties = self._globalConfig.GetProcessByName(self._processName)

        self._originalStdOut = sys.stdout
        self._originalStdErr = sys.stderr
        self._logFile = open(self._processName + ".log", 'w')
    ...
    def Endpoint_Poll(self, inputInfo):
        ...
        while (self._isExecuting):
            ...
            self.Endpoint_Prepoll_Hook_Impl(name)
            ...
            self.Endpoint_Postpoll_Hook_Impl(name, len(dpList), lastEpTime, dpList)
    ...
    def Output_PublishDatapoint(self, dp):
        self._comm.AddDatapointToChannel(self._processName, self._processName, dp)

    def Output_PublishDatapointList(self, dpList):
        for i in range(0, len(dpList)):
            self._comm.AddDatapointToChannel(self._processName, self._processName, dpList[i])
    ...
    def Output_InitializeCSVFile(self):
        ...
        for feeder in feederList:
            ...
            filename = str(self._processName) + "_tf_" + str(feedername) + "--" + str(feedername) + ".csv"
            fh = open(filename, "w")
        ...
        for process in processList:
            ...
            filename = str(self._processName) + "_tp_" + str(processname) + "--" + str(processname) + ".csv"
            fh = open(filename, "w")
